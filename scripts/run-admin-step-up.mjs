...
const script = resolve(repoRoot, "scripts/admin-step-up.mjs");

for (const file of [envFile, script]) {
  try {
    await access(file, constants.R_OK);
  } catch {
    process.stderr.write(`admin step-up: missing required file ${file}\n`);
    process.exit(1);
  }
}
...
child.on("error", (error) => {
  process.stderr.write(`admin step-up failed: ${error.message}\n`);
  process.exitCode = 1;
});
