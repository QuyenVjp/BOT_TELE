...
const script = resolve(repoRoot, "scripts/admin-inventory-import.mjs");

for (const file of [envFile, script]) {
  try {
    await access(file, constants.R_OK);
  } catch {
    process.stderr.write(`production inventory import: missing required file ${file}\n`);
    process.exit(1);
  }
}
...
const child = spawn(process.execPath, [`--env-file=${envFile}`, script, ...process.argv.slice(2)], {
  cwd: repoRoot,
  stdio: "inherit",
  env: childEnv,
});
child.on("error", () => {
  process.stderr.write("production inventory import failed to start\n");
  process.exitCode = 1;
});
