...
import { createBroadcast, enqueueBroadcastRecipients } from "../src/modules/notification/service.js";
import { startPostgresContainer } from "../tests/helpers/pg-container.js";

const counts = (process.argv.slice(2).length ? process.argv.slice(2) : ["1000", "10000"]).map((value) => {
...
try {
  ...
  if (migrated.status !== 0) throw new Error(`QA migration failed: ${migrated.stderr || migrated.stdout}`);

  for (const count of counts) {
    await sql`truncate table notification_delivery, notification_campaign, notification_preference, notification_rate_slot, channel_identity, customer cascade`.execute(ctx.db);
    ...
    for (let i = 0; i < count; i++) {
      ...
      await sql`insert into channel_identity(id,customer_id,channel,channel_user_id) values (${newId()},${customerId},'TELEGRAM',${`chat-${i}`})`.execute(ctx.db);
    }
    ...
    while (true) {
      ...
      const status = (await sql<{ remaining: string }>`select count(*)::text as remaining from notification_delivery where status in ('PENDING','RETRY')`.execute(ctx.db)).rows[0];
      if (status?.remaining === "0") break;
    }

    queueWaits.sort((a, b) => a - b);
    const duplicateSends = [...sentAt.values()].reduce((sum, sends) => sum + Math.max(0, sends.length - 1), 0);
    const sentRows = Number((await sql<{ sent: string }>`select count(*)::text as sent from notification_delivery where status='SENT'`.execute(ctx.db)).rows[0]?.sent ?? 0);
