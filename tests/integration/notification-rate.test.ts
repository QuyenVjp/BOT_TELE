...
} from "../../src/modules/notification/rate-limit.js";
...
async function seedDelivery(chatId: string, suffix = chatId): Promise<string> {
  ...
  await sql`insert into notification_campaign(id,class,content,status,created_by,idempotency_key) values (${campaignId},'CRITICAL_SERVICE','hello','QUEUED','test',${`idem-${suffix}-${campaignId}`})`.execute(
    ctx.db,
  );
  await sql`insert into notification_delivery(id,campaign_id,customer_id,chat_id) values (${deliveryId},${campaignId},${customerId},${chatId})`.execute(
    ctx.db,
  );
  return deliveryId;
}
...
async function resetNotifications(): Promise<void> {
  await sql`truncate table notification_delivery, notification_campaign, notification_rate_slot, customer cascade`.execute(
    ctx.db,
  );
}
...
describe("persistent notification rate", () => {
  ...
  it("waits on a global pause after slots were already reserved", async () => {
    ...
    await runNotificationDeliveryLane({
      ...
      sleep: async (ms) => {
        ...
        if (!paused) {
          paused = true;
          await pauseNotificationRate(ctx.db, 3);
        } else if (ms >= 2500)
          await sql`update notification_rate_slot set next_at=now()-interval '1 second' where scope='pause'`.execute(
            ctx.db,
          );
      },
    });
    ...
    expect(
      (
        await sql<{
          sent: string;
        }>`select count(*) filter (where status='SENT')::text as sent from notification_delivery`.execute(
          ctx.db,
        )
      ).rows[0],
    ).toEqual({ sent: "2" });
  ...
  it("does not send stale leases and restart excludes already-sent deliveries", async () => {
    ...
    await sql`update notification_delivery set status='SENT', sent_at=now(), claim_expires_at=null where id=${sentBeforeRestart}`.execute(
      ctx.db,
    );
    await sql`update notification_delivery set claim_expires_at=now()-interval '1 second' where id=${stale}`.execute(
      ctx.db,
    );
    await sql`update notification_delivery set status='PENDING', claim_expires_at=null, next_attempt_at=now() where id=${stillDue}`.execute(
      ctx.db,
    );
    ...
    const rows = await sql<{
      id: string;
      status: string;
    }>`select id,status from notification_delivery where id in (${sentBeforeRestart},${stale},${stillDue}) order by id`.execute(
      ctx.db,
    );
    expect(rows.rows).toEqual(
      [
        { id: sentBeforeRestart, status: "SENT" },
        { id: stale, status: "RETRY" },
        { id: stillDue, status: "SENT" },
