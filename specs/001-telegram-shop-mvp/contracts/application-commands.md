> **HISTORICAL SPEC.** Mini App/WebApp/`initData` delivery is cancelled by owner. Canonical delivery is a protected Telegram message; `/d/:token` remains a recovery compatibility route only. See `docs/architecture/telegram-only-commerce.md`.
...
| `DeliverCredentialsInTelegram` | verified paid Order + bound customer/chat                             | protected Telegram delivery message                                                                                                                                              | `ORDER_NOT_PAID`, `NOT_OWNER`, `DELIVERY_RETRYABLE`, `DELIVERY_NEEDS_REVIEW`                                   |
| `RevealDeliveryBundle`         | legacy opaque token + explicit owner-bound session                    | one-time compatibility/recovery reveal response                                                                                                                                  | `INVALID_TOKEN`, `NOT_OWNER`, `EXPIRED`, `ALREADY_CONSUMED`, `REVOKED`, `WRONG_AUDIENCE`, `REPLAYED_SESSION`   |
