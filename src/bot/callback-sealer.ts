...
async function parseLegacyCallback(
  ...
  for (const [prefix, action] of [
    ...
    if (value.startsWith(prefix)) {
      ...
      return isId(resourceId) ? { action, resourceId } : null;
    }
  }
  ...
  const adminMatch = /^admin:(\d{1,3}):([0-9A-HJKMNP-TV-Z]{26})$/.exec(value);
  if (adminMatch) {
    const option = Number(adminMatch[1]);
    const resourceId = adminMatch[2]!;
    if (option <= 255 && isId(resourceId)) return { action: "ADMIN_COMMAND", resourceId, option };
  }
  return null;
}
