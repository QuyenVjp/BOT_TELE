 ...
 *     bound to this admin AND this category (atomic, single-use) for the
 *     mutating step, or prove one exists for the preview step;
...
export async function authorizeSensitiveAdminAction(
  ...
  const refuse = async (
    ...
    await appendSensitiveAudit(deps.db, input, actorId, category, code);
