...
export type WalletPurchaseResult =
  ...
  | { ok: false; code: "NOT_FOUND" | "NOT_OWNED" | "ALREADY_PAID" | "INSUFFICIENT_FUNDS" | "ORDER_NOT_PAYABLE"; message: string };
  ...
  return {
    async purchase(input: WalletPurchaseInput): Promise<WalletPurchaseResult> {
      return withTransaction(db, async (trx) => {
        ...
        if (order.customerId !== input.customerId) return { ok: false, code: "NOT_OWNED", message: "Bạn không sở hữu đơn hàng này." };
        if (order.status === "PAID" || order.status === "PROCESSING" || order.status === "COMPLETED") {
          return { ok: true, kind: "ALREADY_PAID", orderId: order.id };
        }
        if (order.status !== "PENDING_PAYMENT") {
          return { ok: false, code: "ORDER_NOT_PAYABLE", message: "Đơn hàng không thể thanh toán bằng ví." };
        }
        if (order.expiresAt !== null && new Date(order.expiresAt).getTime() <= Date.now()) {
          return { ok: false, code: "ORDER_NOT_PAYABLE", message: "Đơn hàng đã hết hạn thanh toán." };
        }
        const variant = await sql<{ variant_active: boolean; product_active: boolean; category_active: boolean; stock_policy: string }>`
        ...
        if (!live || !live.variant_active || !live.product_active || !live.category_active || !isFeature001SellablePolicy(live.stock_policy)) {
          return { ok: false, code: "ORDER_NOT_PAYABLE", message: "Sản phẩm không còn bán." };
        }
        const hasReservation = await orderHasActiveReservation(trx, order.id, order.variantId, new Date());
        if (!hasReservation) return { ok: false, code: "ORDER_NOT_PAYABLE", message: "Sản phẩm không còn tồn kho." };
        ...
        if (!debit.ok) return { ok: false, code: mapLedgerError(debit.code), message: debit.message };

        await voidLiveIntentsForOrder(trx, order.id);
        const paid = await transitionOrder(trx, order, "PAID", "WALLET_PURCHASE", input.correlationId, {
          type: "SYSTEM",
          id: "wallet",
        });
...
function mapLedgerError(code: WalletLedgerErrorCode): "NOT_FOUND" | "INSUFFICIENT_FUNDS" | "ORDER_NOT_PAYABLE" {
