...
export type RegisterResaleEvidenceResult =
  ...
  | { ok: false; code: "INVALID_INPUT" | "NOT_FOUND" | "ACTIVE_EVIDENCE_EXISTS" | "CONFLICT"; message: string };
...
export type PublishProductResult =
  ...
  | { ok: false; code: "NOT_FOUND" | "NOT_READY" | "VERSION_CONFLICT" | "CONFLICT"; message: string };
...
function validMetadata(value: Record<string, string | number | boolean | null> | undefined): boolean {
...
function publicationVersion(productVersion: number, variants: Array<{ id: string; version: number; evidenceId: string | null }>): string {
  return `${productVersion}:${variants.map((v) => `${v.id}:${v.version}:${v.evidenceId ?? "-"}`).sort().join(",")}`;
}
...
export async function getProductPublicationReadiness(
  ...
  const result = await sql<{
    ...
    select p.id as product_id, p.version as product_version, p.is_active as product_active,
     ...
     group by p.id, p.version, p.is_active, p.is_archived, p.is_test, v.id, v.version,
  ...
  const variants = result.rows
    ...
    .map((row) => {
      ...
      return {
        ...
        published: row.publication_evidence_id === row.evidence_id
...
export async function registerResaleEvidenceInTransaction(
  ...
  if (!source || !reference || !summary || !reason || !requestId || !validMetadata(input.metadataRedacted) || !CODE_RE.test(reference)) {
    return { ok: false, code: "INVALID_INPUT", message: "Bằng chứng hoặc lý do không hợp lệ." };
  }
  if (!isId(input.variantId)) return { ok: false, code: "NOT_FOUND", message: "Không tìm thấy biến thể." };
  const existingRequest = await sql<{ id: string; variant_version: number }>`
    select id, version as variant_version from resale_evidence re join product_variant v on v.id = re.variant_id
     where re.registration_request_id = ${requestId} limit 1
  `.execute(exec);
  if (existingRequest.rows[0]) return { ok: true, evidenceId: existingRequest.rows[0].id, variantVersion: existingRequest.rows[0].variant_version, replayed: true };
  ...
  `.execute(exec);
  if (!variant.rows[0]) return { ok: false, code: "NOT_FOUND", message: "Không tìm thấy biến thể." };
  const active = await sql<{ id: string }>`
    select id from resale_evidence where variant_id = ${input.variantId} and status = 'ACTIVE' limit 1
  `.execute(exec);
  if (active.rows[0]) return { ok: false, code: "ACTIVE_EVIDENCE_EXISTS", message: "Biến thể đã có bằng chứng ACTIVE." };
  ...
  try {
    ...
    return { ok: false, code: "CONFLICT", message: "Không thể đăng ký bằng chứng hiện tại." };
  }
}

export async function registerResaleEvidence(db: Db, input: RegisterResaleEvidenceInput): Promise<RegisterResaleEvidenceResult> {
...
export async function publishProductInTransaction(
  ...
  if (!readiness.canPublish) return { ok: false, code: "NOT_READY", message: `Chưa đủ điều kiện: ${readiness.blockers.join(", ")}.` };
  if (readiness.publicationVersion !== input.expectedPublicationVersion) {
    return { ok: false, code: "VERSION_CONFLICT", message: "Sản phẩm đã thay đổi. Vui lòng mở lại readiness." };
  }
  const activeVariants = readiness.variants.filter((variant) => variant.active);
  if (activeVariants.every((variant) => variant.published)) {
    return { ok: true, kind: "REPLAYED", productId: input.productId, publicationVersion: input.expectedPublicationVersion };
  }
  const updated = await sql<{ id: string }>`
    update product_variant v
     ...
     where v.product_id = p.id and p.id = ${input.productId}
       ...
       and v.is_active
       and v.version in (${sql.join(readiness.variants.filter((variant) => variant.active).map((variant) => sql`${variant.version}`), sql`, `)})
     returning v.id
  `.execute(exec);
  if (updated.rows.length !== readiness.variants.filter((variant) => variant.active).length) {
    return { ok: false, code: "VERSION_CONFLICT", message: "Biến thể đã thay đổi. Vui lòng mở lại readiness." };
  }
  await appendAuditEvent(exec, {
    ...
    reason: input.reason,
    correlationId: input.correlationId,
    metadataRedacted: { publicationVersion: input.expectedPublicationVersion, variants: readiness.variants.filter((variant) => variant.active).length },
  });
  return { ok: true, kind: "PUBLISHED", productId: input.productId, publicationVersion: input.expectedPublicationVersion };
}

export async function publishProduct(db: Db, input: PublishProductInput): Promise<PublishProductResult> {
